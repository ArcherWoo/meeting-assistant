...
- 先保留结构，再做语义增强
- 附件模式与知识库导入共用同一套解析能力
- 不允许把 Excel / Word / PDF 一律摊平成无结构长文本
- 不把 LLM 当底层解析器；LLM 只用于可选语义增强

### 18.2 当前支持的结构化能力

#### Excel

- 需要保留 merged cells 信息
- 需要恢复多层表头与 `header_path`
- 需要把 sheet、row range 等定位信息传递到检索与引用层

#### Word

- 需要保留 heading / paragraph / table 的原始顺序
- 表格需要进入结构化表模型，而不是只保留纯文本

#### PDF

- 优先做 layout-aware 解析
- 当页面是扫描件时，允许自动走 OCR
- OCR 后的结果不应只是一整页大段文本，而应继续切成标题 / 段落 / 列表
- 对明显的 OCR 表格，系统应尝试恢复成结构化表格

#### 图片

- 应支持图片元数据读取
- 应支持可选 OCR
- OCR 能力与扫描 PDF 共用一套底层工具

### 18.3 引用定位体验

知识库命中与上下文引用不再只展示“文件名 + 片段号”。

产品上现在要求前端在 citation 中尽量展示来源定位信息，包括但不限于：
...
目标是让用户知道“这条结论来自哪一页、哪个 sheet、哪段 OCR 或哪个表格”，而不是只能看到模糊片段。

### 18.4 依赖与降级策略

OCR 与高级 PDF 解析允许依赖外部组件，但产品行为必须满足：

- 有依赖时，自动增强
- 无依赖时，优雅降级
- 降级不能阻断附件上传、文本提取与知识库导入主流程
- 降级结果必须保留可解释 warning，而不是静默失败

### 18.5 当前实现状态

本 PRD 对应的当前实现已经落地：

- 统一文档解析层 `document_parsing/`
- `XLSX` 结构保真解析
- `DOCX` 块级顺序解析
- layout-aware `PDF` 解析
...
- OCR 文本切块
- OCR 表格恢复雏形
- knowledge chunk locator metadata
- citation locator badges 前端展示
## 2026-04-07 产品补充口径：Know-how 权限与会话隐私
...
- Chat 在回答“规则库里有多少条规则 / 有哪些分类”这类问题时，返回的也必须是当前用户可见范围，不是全库总量。
